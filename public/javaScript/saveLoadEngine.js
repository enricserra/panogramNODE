...
var SaveLoadEngine = Class.create( {
    ...
    createGraphFromSerializedData: function(JSONString, noUndo, centerAround0) {
        ...
        if (!noUndo) {
            ...
            var genderOk = editor.getGraph().setProbandData( probandData.firstName, probandData.lastName, XWiki.sex );
            if (!genderOk)
    ...
    save: function() {
        ...
        var jsonData = PedigreeExport.exportAsSimpleJSON(editor.getGraph().DG, "all");
        ...
        new Ajax.Request("/Panogram/savePedigree/" + XWiki.ped_name + "/?fam=" + jsonData, {
            method: 'GET',
            onCreate: function() {
    ...
    load: function() {
       ...
       var jsonastext=XWiki.family;
        ...
        //var jsonastext = JSON.parse(XWiki.family)

	/*for(var i=0;i<jsonastext.length;i++){
		jsonastext[i].id = jsonastext[i].pedigreeId;
                jsonastext[i].mother = jsonastext[i].motherId;
		if(jsonastext[i].disorderList){
			if(jsonastext[i].disorderList.length>0){jsonastext[i].disorders  = ["affected"];}
	                jsonastext[i].father = jsonastext[i].fatherId;
		}
	 
}*/
        editor.getSaveLoadEngine().createGraphFromImportData(jsonastext, 'simpleJSON', {}, true , true);
